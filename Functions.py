...
def Overall_Heat_Coefficient(Re, RSI, ReT, Pr, k, D, L):  # W/(m^2.K)
    if Re > ReT:  # Turb
        ...
        Gz = (D / L) * Re * Pr  # Graetz number
        Nu = ((3.66 / (tanh(2.264 * (Gz ** (-1 / 3)) + 1.7 * (Gz ** (-2 / 3))))) + (0.0499 * Gz * tanh(Gz ** (-1)))) / (tanh(2.432 * (Pr ** (1 / 6)) * (Gz ** (-1 / 6))))  # Ts constant combined entry length Pr>0.1
        h = Nu * k / D
        U_T = 1 / ((1 / h) + (RSI))
    return U_T
...
def Heat_Loss(U, A, T_Air_Duct, T_ambient):
    A = U * A * (T_Air_Duct - T_ambient)
    return A


def Wall_Temperature_Calc(RSI, A, Q_Watt, T_ambient):
    # Q_Watt>0 with cooling assumption
    if RSI == 0:
        T_Wall = T_ambient
    else:
        U_T = 1 / (RSI)
        T_Wall = T_ambient + (Q_Watt / (A * U_T))
    return T_Wall


def Outlet_Temperature(Q_Watt, Rho_Gas, Volumetric_Flow, Specific_Heat, T_Air_Inlet):
    # Q_Watt>0 with cooling assumption
    T_Outlet = T_Air_Inlet - (Q_Watt / (Rho_Gas * Volumetric_Flow * Specific_Heat))
    return T_Outlet
